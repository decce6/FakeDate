...
import me.decce.transformingbase.core.FakeDateConfig;
...
public class ConfigLoader {
    ...
    public static void save(FakeDateConfig config) {
    ...
    public static FakeDateConfig load() {
        return loadConfig();
    }

    private static FakeDateConfig loadConfig() {
        ...
        return new FakeDateConfig();
    }

    private static CommentedFileConfig toNightConfig(FakeDateConfig config) {
        var night = makeNightConfig();
        try {
            for (Field field : FakeDateConfig.class.getDeclaredFields()) {
                ...
                if (field.isAnnotationPresent(FakeDateConfig.Comment.class)) {
                    night.setComment(field.getName(), field.getAnnotation(FakeDateConfig.Comment.class).value());
                }
            }
        } catch (Exception e) {
    ...
    private static FakeDateConfig fromNightConfig() {
        var config = new FakeDateConfig();
